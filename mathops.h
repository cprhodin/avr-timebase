...
//
static inline uint16_t divu(uint32_t dividend, uint16_t divisor)
{
    uint8_t count = 17;
...
//
// unsigned multiply-divide with an implicit multiply by 2^32
//
...
//
static inline uint32_t mulu(uint16_t multiplier, uint16_t multiplicand)
{
    uint32_t product;
...
{
    return divu(mulu(multiplier, multiplicand), divisor);
}
...
{
    return divu(mulu(multiplier, multiplicand) + (divisor / 2), divisor);
}
